...
시계열 모델에도 Cross-Validation을 적용 할 수 있는데, 시간의 순서로 정렬되어있다는 데이터의 특성을 고려해야한다. 따라서 비시계열 데이터와 달리 적용 형상이 약간 다르고, Non-Dependent Cross-Validation라는 시계열 데이터에서만 사용되는 방식도 있다. 시계열의 다른 Cross-Validation과 함께 해당 방식을 정리해보고자 한다. 

들어가기 전에, 비시계열 데이터에서의 학습/테스트 예시를 살펴보자.
식상하긴 하지만 예를 들어, 사람의 키를 바탕으로 몸무게를 예측하는 모델이 있다고 하자. 15명의 데이터가 있다고 할 때, 학습 데이터는 1 ~ 12번 사람, 테스트 데이터는 13 ~ 15번 사람으로 둔다. 1 ~ 12번 사람의 데이터를 바탕으로 모델을 생성하고, 13 ~ 15번 사람 데이터로 모델을 평가한다. 모델 평가시에 1 ~ 12번 사람의 데이터는 들어가지 않는다. (만약 학습 데이터로 학습을 해놓고, 다시 학습 데이터를 예측하라고 하면 치팅이다. 이미 정답을 한번 알려줬으니 당연히 예측을 잘하지!! 엄연한 치팅이다.) 따라서, 해당 경우 학습 데이터와 테스트 데이터가 독립적이라고 볼 수 있다. 
...
비시계열과 달리 시계열 데이터는 자기상관관계가 있다. 따라서 학습과 테스트 사이의 독립성을 유지 못할 수도 있다.

> 자기상관관계 : 현재의 상태와 과거 & 미래와 밀접한 관련 있음 = 시간의 흐름에 따라 독립적이지 않음

시계열 데이터의 자기상관관계로 인해, 시계열 모델은 자기회귀오차 모형과 비슷한 형태인 경우가 많다. (과거의 연속된 데이터를 통해, 미래 데이터 예측함.)

> 자기회귀오차 모형 예시: y(t+1)=a + b * y(t) + c * y(t-1) + d * y(t-2) + ... 

아래 #1 ~ #3은 위와 같은 자기회귀오차 모형 형태에 적용할 수 있는 Cross-Validation이다. 
아래에서 #1, #2에서는 학습 데이터와 테스트 데이터 사이의 독립성을 유지하지 않는다는 단점이 있는데, 3은 이를 보완한 것이다. 
